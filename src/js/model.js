...
const createRecipeObject = function (data) {
  ...
  return {
    ...
    ...(recipe.key && { key: recipe.key }), //adds key if there is one
  };
};
  ...
  try {
    ...
    state.search.results = data.data.recipes.map(item => {
      return {
        ...
        ...(item.key && { key: item.key }), //adds key if there is one
      };
    });
    state.search.page = 1;
  } catch (err) {
...
export const addBookmark = function (recipe) {
  ...
  // mark current recipe as booklarked
...
export const deleteBookmark = function (id) {
  ...
  // mark current recipe as not booklarked
  ...
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map(ing => {
        ...
        if (ingArr.length !== 3) throw new Error('Wrong number of inputs');
